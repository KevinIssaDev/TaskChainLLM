...
def main():
    ...
    parser.add_argument("--pattern", nargs='*', help="Path(s) to the pattern file(s)")
    ...
    try:
        if len(args.pattern) == 1:
            output = execute_pattern(args.pattern[0], input_data, args.model, allowed_workers)
        else:
            output = pipe_patterns(args.pattern, input_data, args.model, allowed_workers)
        print(output)
